import math
import chess.lib
from chess.lib.utils import encode, decode
from chess.lib.heuristics import evaluate
from chess.lib.core import makeMove

###########################################################################################
# Utility function: Determine all the legal moves available for the side.
# This is modified from chess.lib.core.legalMoves:
#  each move has a third element specifying whether the move ends in pawn promotion
def generateMoves(side, board, flags):
    for piece in board[side]:
        fro = piece[:2]
        for to in chess.lib.availableMoves(side, board, piece, flags):
            promote = chess.lib.getPromote(None, side, board, fro, to, single=True)
            yield [fro, to, promote]
            
###########################################################################################
# Example of a move-generating function:
# Randomly choose a move.
def random(side, board, flags, chooser):
    '''
    Return a random move, resulting board, and value of the resulting board.
    Return: (value, moveList, boardList)
      value (int or float): value of the board after making the chosen move
      moveList (list): list with one element, the chosen move
      moveTree (dict: encode(*move)->dict): a tree of moves that were evaluated in the search process
    Input:
      side (boolean): True if player1 (Min) plays next, otherwise False
      board (2-tuple of lists): current board layout, used by generateMoves and makeMove
      flags (list of flags): list of flags, used by generateMoves and makeMove
      chooser: a function similar to random.choice, but during autograding, might not be random.
    '''
    moves = [ move for move in generateMoves(side, board, flags) ]
    if len(moves) > 0:
        move = chooser(moves)
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])
        value = evaluate(newboard)
        return (value, [ move ], { encode(*move): {} })
    else:
        return (evaluate(board), [], {})

###########################################################################################
# Stuff you need to write:
# Move-generating functions using minimax, alphabeta, and stochastic search.
def minimax(side, board, flags, depth):
    '''
    Return a minimax-optimal move sequence, tree of all boards evaluated, and value of best path.
    Return: (value, moveList, moveTree)
      value (float): value of the final board in the minimax-optimal move sequence
      moveList (list): the minimax-optimal move sequence, as a list of moves
      moveTree (dict: encode(*move)->dict): a tree of moves that were evaluated in the search process
    Input:
      side (boolean): True if player1 (Min) plays next, otherwise False
      board (2-tuple of lists): current board layout, used by generateMoves and makeMove
      flags (list of flags): list of flags, used by generateMoves and makeMove
      depth (int >=0): depth of the search (number of moves)
    '''
    moveList = []
    moveTree = {}

    if depth == 0:
      return (evaluate(board), moveList, moveTree)

    possibleMoves = list(generateMoves(side, board, flags))
    best = possibleMoves[0]
    currBest = []

    if side:
      heuristic = math.inf

      for move in generateMoves(side,board,flags):
        moveEnc = encode(*move)
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])
        val1, list1, tree1 = minimax(newside, newboard, newflags, depth -1)
        moveTree[moveEnc] = tree1
        if heuristic > val1:
          heuristic = val1
          best = move
          currBest = list1

      moveList = [best] + currBest
      return (heuristic, moveList, moveTree)

    else:

      heuristic = -math.inf

      for move in generateMoves(side,board,flags):
        moveEnc = encode(*move)
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])
        val1, list1, tree1 = minimax(newside, newboard, newflags, depth -1)
        moveTree[moveEnc] = tree1
        if heuristic < val1:
          heuristic = val1
          best = move
          currBest = list1

      moveList = [best] + currBest
      return (heuristic, moveList, moveTree)


    return (value, moveList, moveTree)

def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    '''
    Return minimax-optimal move sequence, and a tree that exhibits alphabeta pruning.
    Return: (value, moveList, moveTree)
      value (float): value of the final board in the minimax-optimal move sequence
      moveList (list): the minimax-optimal move sequence, as a list of moves
      moveTree (dict: encode(*move)->dict): a tree of moves that were evaluated in the search process
    Input:
      side (boolean): True if player1 (Min) plays next, otherwise False
      board (2-tuple of lists): current board layout, used by generateMoves and makeMove
      flags (list of flags): list of flags, used by generateMoves and makeMove
      depth (int >=0): depth of the search (number of moves)
    '''
    moveTree = {}
    moveList = []

    if depth == 0:
      return (evaluate(board), moveList, moveTree)

    moves = list(generateMoves(side, board, flags))
    currBest = moves[0]

    if side:
      heuristic = math.inf
      bestList = []
      for move in generateMoves(side,board,flags):
        moveEnc = encode(*move)
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])
        val1, list1, tree1 = alphabeta(newside, newboard,newflags, depth-1, alpha, beta )
        moveTree[moveEnc] = tree1

        if val1 < heuristic:
          heuristic = val1
          currBest = move
          bestList = list1
        moveList = [currBest] + bestList
        beta = min(heuristic, beta)
        if beta <= alpha:
          break
      return (heuristic, moveList, moveTree)

    else:
      heuristic = -math.inf
      bestList = []
      for move in generateMoves(side,board,flags):
        moveEnc = encode(*move)
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])
        val1, list1, tree1 = alphabeta(newside, newboard,newflags, depth-1, alpha, beta )
        moveTree[moveEnc] = tree1

        if val1 > heuristic:
          heuristic = val1
          currBest = move
          bestList = list1
        moveList = [currBest] + bestList
        alpha = max(heuristic, alpha)
        if beta <= alpha:
          break
      return (heuristic, moveList, moveTree)




def stochastic(side, board, flags, depth, breadth, chooser):
    '''
    Choose the best move based on breadth randomly chosen paths per move, of length depth-1.
    Return: (value, moveList, moveTree)
      value (float): average board value of the paths for the best-scoring move
      moveLists (list): any sequence of moves, of length depth, starting with the best move
      moveTree (dict: encode(*move)->dict): a tree of moves that were evaluated in the search process
    Input:
      side (boolean): True if player1 (Min) plays next, otherwise False
      board (2-tuple of lists): current board layout, used by generateMoves and makeMove
      flags (list of flags): list of flags, used by generateMoves and makeMove
      depth (int >=0): depth of the search (number of moves)
      breadth: number of different paths 
      chooser: a function similar to random.choice, but during autograding, might not be random.
    '''
    moveList = []
    moveTree = {}
    heuristic = 0
    moves = list(generateMoves(side, board, flags))
    currBest = moves[0]

    if side == True:
      heuristic = math.inf
      for move in moves:
        sumVals = 0
        moveEnc = encode(*move)
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])

        i = 0
        while i < breadth:
          val1, tree1 = helper_stoch(newside, newboard, newflags, depth -1, breadth, chooser)
          sumVals += val1

          if moveEnc in moveTree:
            moveTree[moveEnc].update(tree1)
          else:
            moveTree[moveEnc] = tree1

          i += 1 
        avg = sumVals / breadth
        if avg < heuristic:
          heuristic = avg
          currBest = move
    else:
      heuristic = -math.inf
      for move in moves:
        sumVals = 0
        moveEnc = encode(*move)
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])

        i = 0
        while i < breadth:
          val1, tree1 = helper_stoch(newside, newboard, newflags, depth -1, breadth, chooser)
          sumVals += val1

          if moveEnc in moveTree:
            moveTree[moveEnc].update(tree1)
          else:
            moveTree[moveEnc] = tree1

          i += 1

        avg = sumVals / breadth
        if avg > heuristic:
          heuristic = avg
          currBest = move

    return (heuristic, [currBest], moveTree)
    
   


def helper_stoch(side, board, flags, depth, breadth, chooser):

  moveTree = {}

  if depth == 0:
    return (evaluate(board), moveTree)

  moves = list(generateMoves(side, board, flags))
  move = chooser(moves)
  moveEnc = encode(*move)

  newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])
  val1, tree1 = helper_stoch(newside, newboard, newflags, depth - 1, breadth, chooser)

  moveTree[moveEnc] = tree1

  return (val1, moveTree)
